...
const FieldInlineInput = ({ input, placeholder, measurement, type, meta, test }) => {
  const empty = input.value.length < 2
  const error = (meta.modified && meta.error) || (meta.touched && meta.error)
  return (
    <div className={cn(css.container, { [ css.hide ]: !measurement }, { [css.text]: type !== 'number' }, { [css.error]: error })} >
      <input type={type}
